...
| `recompactAfterTokens` | 50000 | After a compaction or a skipped one (pruning would free too little) leaves context above the threshold, ask again once context has grown this many tokens past where it then stood. 0 waits until context drops under the threshold, which it never does on its own |
